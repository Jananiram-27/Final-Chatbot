...
// --- PUDHUSA ADD PANNA VENDIYA LINE ---
...
const generateChatResponse = async (req, res) => {
  try {
    ...
    // --- START OF YOUR CUSTOM AI ENGINE ---
    let aiResponse;
    let vaderScore = 0;
    
    try {
      console.log('🧠 Generating AI response using Custom VADER Engine...');
      // Neenga train panna engine-kitta message-a kudukkurom
      const customAiResult = getAiResponse(message); 
      
      aiResponse = customAiResult.reply;
      vaderScore = customAiResult.score;
      console.log(`✅ Custom AI replied! (VADER Score: ${vaderScore})`);
      
    } catch (error) {
      console.error('Custom Engine Error:', error);
      aiResponse = "I hear you. Can you tell me more about how you're feeling?";
    }
    // --- END OF YOUR CUSTOM AI ENGINE ---
   ...
   res.status(200).json({
      ...
      aiGenerated: true,
      sentimentScore: vaderScore // <-- Ithu frontend-la graph poda use aagum
    });

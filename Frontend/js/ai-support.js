document.addEventListener('DOMContentLoaded', function() {
    ...
    async function sendMessage() {
        ...
        try {
            ...
            const aiData = await generateAIResponse(message);
            hideTypingIndicator();
 ...
 // Moondravathu parameter 'vaderScore' add pannirukoom
    function addMessage(text, sender, vaderScore = null) {
        ...
        if (sender === 'ai') {
            ...
            if (vaderScore !== null && vaderScore !== undefined) {
                formattedText += `<br><br><span style="display: block; border-top: 1px solid #eee; padding-top: 5px; color: #888; font-size: 11px; font-style: italic;">VADER Sentiment: ${vaderScore}</span>`;
            }
    ...
    async function generateAIResponse(userMessage) {
        try {
           ...
           const data = await response.json();
            ...
            if (data.success) {
                // Ippo namma text-a mattum anuppala, sentimentScore-yum anupurom!
                return { 
                    text: data.response || data.reply, // Backend sends 'reply' or 'response'
                    score: data.sentimentScore !== undefined ? data.sentimentScore : null 
                };
            } else {
